 ...
 *
 * <p><b>Format:</b> 1 sign bit · 5 exponent bits (bias 15) · 10 mantissa bits.
 * Range: ±65504 · ~3.31 decimal digits of precision · stored as 2 bytes.
 *
 * <p><b>Why use it?</b>
 * <ul>
 *   <li><em>Memory bandwidth.</em> A {@code float16[]} is half the size of {@code float[]}.
 *       At 100 M elements that is 200 MB vs 400 MB — a decisive factor when arrays
 *       don't fit in CPU cache or GPU VRAM.</li>
 *   <li><em>ML / AI inference.</em> Neural-network weights and activations are routinely
 *       stored in float16 after training. Modern GPUs and NPUs have dedicated float16
 *       tensor units that deliver 2–8× the throughput of float32 on the same silicon.</li>
 *   <li><em>Sensor and telemetry streams.</em> Temperature, pressure, gyroscope readings
 *       rarely need more than 3 significant digits. float16 encodes them in half the wire
 *       bandwidth of float32 with no observable accuracy loss for the application.</li>
 *   <li><em>HDR and graphics.</em> OpenGL / Vulkan / Metal half-float textures and
 *       render targets cut VRAM usage by 2× while covering the full visible luminance
 *       range.</li>
 * </ul>
 *
 * <p><b>Trade-offs to understand before using it:</b>
 * <ul>
 *   <li>Precision is ~3.31 digits — not suitable for financial arithmetic or scientific
 *       simulation that requires float32's ~7.2 digits.</li>
 *   <li>Maximum finite value is 65504. Values beyond overflow to ±Infinity.</li>
 *   <li>Arithmetic promotes to float32 internally; the result is then rounded back to
 *       float16. There is no hardware float16 ALU on the JVM today, so throughput
 *       for scalar operations is not faster than float32.</li>
 *   <li>The benefit is almost exclusively in <em>storage density and I/O bandwidth</em>,
 *       not in per-operation compute speed on the JVM.</li>
 * </ul>
 *
 * <p>Backed by {@link Float#floatToFloat16} and {@link Float#float16ToFloat} (Java 20+).
 * Compatible with the Java Vector API's {@code FLOAT16} element species.
 */
